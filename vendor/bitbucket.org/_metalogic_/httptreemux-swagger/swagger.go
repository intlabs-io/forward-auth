...
// WrapHandler wraps swaggerFiles.Handler and returns httptreemux.HandlerFunc
...
// Handler wraps `http.Handler` into `httptreemux.HandlerFunc`.
func Handler(configFns ...func(*Config)) httptreemux.HandlerFunc {
	config := &Config{
		URL:          "doc.json",
		DeepLinking:  true,
		DocExpansion: "list",
	...
	if err != nil {
		log.Errorf("Error parsing index template: %s", err)
	}
	...
	return func(w http.ResponseWriter, r *http.Request, params map[string]string) {
		...
		log.Debugf("path = %s, prefix = %s", path, prefix)
		...
		case "index.html":
			...
			if err != nil {
				log.Errorf("Error executing index template: %s", err)
			}
		case "doc.json":
			w.Header().Set("Content-Type", "application/json; charset=utf-8")
			doc, err := swag.ReadDoc()
			log.Debugf("Read doc: %s", doc)
			if err != nil {
				log.Errorf("Read doc failed: %s", err)
			}
