...
//   - Checks: a collection of group/path checks with access rules
...
func (acs *AccessSystem) String() string {
	t, err := json.MarshalIndent(acs, "", "   ")
	if err != nil {
		return `{"acs", null}`
	}
	return fmt.Sprintf(`{"acs": %s}`, string(t))
}
...
type Authorization struct {
	...
	GroupChecks []GroupChecks     `json:"groupchecks"`
}
...
func (h Group) Validate() error {
	return validation.ValidateStruct(&h,
		validation.Field(&h.Groupname, validation.Required, validation.Length(3, 256)),
	)
}

// GroupChecks associates a set of checks with groups to which they apply
...
func (hg GroupChecks) Validate() error {
...
// - Path is the path
// - Rules maps action strings ("CREATE", "DELETE", ...) to a rule governing the action
type Path struct {
...
// Rule defines an expression that is evaluated to make an access decision.
// If MustAuth is true and valid credentials are not present the rule handler
// returns false without performing expresion evaluation
type Rule struct {
