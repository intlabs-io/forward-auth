...
const (
	// host is treated specially because Go may not include it in the
	// request header map on the server side of a request.
	hostHeader        = "Host"
	forwardHostHeader = "X-Forwarded-Host"
)
...
// NewForwardAuthVerifier verifies a Traefik auth request. It returns an error if the HTTP
// Signature parameters are not present in any headers, are present in more than
// one header, are malformed, or are missing required parameters. It ignores
// unknown HTTP Signature parameters.
func NewForwardAuthVerifier(h http.Header, method, path, rawQuery string) (Verifier, error) {
	if host, hasForwardHostHeader := h[forwardHostHeader]; hasForwardHostHeader {
		h[hostHeader] = host
	}
	verifier, err := newVerifier(h, func(h http.Header, toInclude []string, created int64, expires int64) (string, error) {
		return signatureString(h, toInclude, includeRequestTarget(method, path, rawQuery), created, expires)
	})
	if err == nil {
		log.Debugf("forward-auth verifier: %+v", verifier)
	}
	return verifier, err
}
