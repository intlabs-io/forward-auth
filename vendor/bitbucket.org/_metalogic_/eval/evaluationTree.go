...
type evaluationFunction func(left interface{}, right interface{}, parameters Parameters) (interface{}, error)
...
type EvaluationTree struct {
	symbol OperatorSymbol // eg AND, OR ...

	left, right *EvaluationTree

	// the operation that will be used to evaluate this stage (such as adding [left] to [right] and return the result)
	operator evaluationFunction

	// ensures that both left and right values are appropriate types for this stage. Returns an error if they aren't operable.
...
func (t *EvaluationTree) UnmarshalJSON(data []byte) error {
	...
	if err = unmarshalMap(rawJSON, t); err != nil {
		return err
	}
	return nil
}

func unmarshalMap(rawJSON map[string]interface{}, tree *EvaluationTree) (err error) {
	var v interface{}
	v = rawJSON["type"]
	...
	if v != nil {
		...
		case "DATE":
			...
			v = rawJSON["value"]
			if v != nil {
				tree.operator = makeDateStage(v)
			} else {
				return fmt.Errorf("a date literal found with no value")
			}
		case "DATETIME":
			tree.symbol = LITERAL
			v = rawJSON["value"]
			if v != nil {
				tree.operator = makeDatetimeStage(v)
			} else {
				return fmt.Errorf("a date literal found with no value")
			}
		case "FUNCTION":
			tree.symbol = FUNCTIONAL
			v = rawJSON["value"]
			if v != nil {
				if name, ok := v.(string); ok {
					if f, found := Builtins[name]; found {
						log.Debugf("processing func %s\n", name)
						tree.operator = makeFunctionStage(f)
						v = rawJSON["right"]
						if v != nil {
							if right, ok := v.(map[string]interface{}); ok {
								tree.right = &EvaluationTree{}
								if err = unmarshalMap(right, tree.right); err != nil {
									return err
								}
							} else {
								return fmt.Errorf("invalid right subtree %v", v)
							}
						}
					} else {
					...
					return fmt.Errorf("invalid function name %v", v)
				}
			} else {
				return fmt.Errorf("function found with no value")
			}
		case "LITERAL":
			tree.symbol = LITERAL
			v = rawJSON["value"]
			if v != nil {
				tree.operator = makeLiteralStage(v)
			} else {
				return fmt.Errorf("a literal found with no value")
			}
		case "OPERATOR":
			if v != nil {
				...
				v = rawJSON["value"]
				if v != nil {
					symbol, err := Symbol(v.(string))
					if err != nil {
						return err
					}
					tree.symbol = symbol
					tree.operator = operatorEvalMap[symbol]

					v = rawJSON["left"]

					if v != nil {
						if left, ok := v.(map[string]interface{}); ok {
							tree.left = &EvaluationTree{}
							if err = unmarshalMap(left, tree.left); err != nil {
								return err
							}
						} else {
							return fmt.Errorf("invalid left subtree %v", v)
						}
					}
					v = rawJSON["right"]
					if v != nil {
				...
				return fmt.Errorf("an operator found with no value")
			}
		case "REFERENCE":
			tree.symbol = ACCESS
			v = rawJSON["value"]
			if v != nil {
				r := rawJSON["right"]
				if r != nil {
					if right, ok := r.(map[string]interface{}); ok {
						tree.right = &EvaluationTree{}
						if err = unmarshalMap(right, tree.right); err != nil {
							return err
						}
					} else {
						return fmt.Errorf("invalid reference right subtree %v", v)
					}
				}

				tree.operator = makeReferenceStage(v.(string))
			} else {
				return fmt.Errorf("a reference found with no value")
			}
		}
...
func (t *EvaluationTree) String() string {
	...
	return fmt.Sprintf("{\"symbol\": %s, \"operator\": %v, \"left\": %v, \"right\": %v}", t.symbol, t.operator, t.left, t.right)
...
// treeFromExpression returns an evaluation tree for a given expression string
func treeFromExpression(expression string, functions map[string]ExpressionFunction) (ast *EvaluationTree, err error) {
...
func (stage *EvaluationTree) swapWith(other *EvaluationTree) {

	temp := *other
	other.setToNonStage(*stage)
	stage.setToNonStage(temp)
}

func (stage *EvaluationTree) setToNonStage(other EvaluationTree) {

	stage.symbol = other.symbol
	stage.operator = other.operator
	stage.leftTypeCheck = other.leftTypeCheck
	stage.rightTypeCheck = other.rightTypeCheck
	stage.typeCheck = other.typeCheck
	stage.typeErrorFormat = other.typeErrorFormat
}

func (stage *EvaluationTree) isShortCircuitable() bool {
...
func makeParameterStage(parameterName string) evaluationFunction {
...
func makeLiteralStage(literal interface{}) evaluationFunction {
	return func(left interface{}, right interface{}, parameters Parameters) (interface{}, error) {
		return literal, nil
	}
}

func makeFunctionStage(function ExpressionFunction) evaluationFunction {
...
// handles a yyyy-mm-dd date or RFC3339 datetime literal
func makeDateStage(literal interface{}) evaluationFunction {
	return func(left interface{}, right interface{}, parameters Parameters) (interface{}, error) {
		var dt time.Time
		var err error
		if s, ok := literal.(string); ok {
			...
			if err != nil {
				...
				return float64(dt.UnixNano()), nil
			}
			return float64(dt.UnixNano()), nil
		}
		return literal, fmt.Errorf("can't parse %v as datetime", literal)
	}
}

// handles a yyyy-mm-dd date or RFC3339 datetime literal
func makeDatetimeStage(literal interface{}) evaluationFunction {
	return func(left interface{}, right interface{}, parameters Parameters) (interface{}, error) {
		var dt time.Time
		var err error
		if s, ok := literal.(string); ok {
...
func makeAccessorStage(pair []string) evaluationFunction {
...
func makeReferenceStage(ref string) evaluationFunction {
	...
	return func(left interface{}, right interface{}, parameters Parameters) (ret interface{}, err error) {
		...
		value, err := parameters.Get(path[0])
		if err != nil {
			return nil, err
		}
		...
		defer func() {
			if r := recover(); r != nil {
				errorMsg := fmt.Sprintf("Failed to access '%s': %v", ref, r.(string))
				err = errors.New(errorMsg)
				ret = nil
			}
		}()
		...
		for i := 1; i < len(path); i++ {
			...
			if coreValue.Kind() != reflect.Struct {
				return nil, errors.New("Unable to access '" + path[i] + "', '" + path[i-1] + "' is not a struct")
			}

			field := coreValue.FieldByName(path[i])
			if field != (reflect.Value{}) {
				value = field.Interface()
				continue
			}

			method := coreValue.MethodByName(path[i])
			if method == (reflect.Value{}) {
				if corePtrVal.IsValid() {
					method = corePtrVal.MethodByName(path[i])
				}
				if method == (reflect.Value{}) {
					return nil, errors.New("No method or field '" + path[i] + "' present on parameter '" + path[i-1] + "'")
				}
			}
			switch right.(type) {
			case []interface{}:
			...
			case interface{}:
				...
				params[0] = reflect.ValueOf(givenParams)
			...
			if err != nil {
				return nil, errors.New("Method call failed: '" + path[0] + "." + path[1] + "': " + err.Error())
			}
			...
			if retLength == 0 {
				return nil, errors.New("Method call failed: '" + path[i-1] + "." + path[i] + "' did not return any values.")
			}
			...
			return nil, errors.New("Method call failed: '" + path[0] + "." + path[1] + "' did not return either one value, or a value and an error. Cannot interpret meaning.")
		}
...
func containsStage(left interface{}, right interface{}, parameters Parameters) (interface{}, error) {
	...
	for _, value := range right.([]interface{}) {
		if strings.Contains(left.(string), value.(string)) {
			return true, nil
		}
	}
	return false, nil
...
func endsWithStage(left interface{}, right interface{}, parameters Parameters) (interface{}, error) {

	for _, value := range right.([]interface{}) {
		if strings.HasSuffix(left.(string), value.(string)) {
			return true, nil
		}
	}
	return false, nil
...
func startsWithStage(left interface{}, right interface{}, parameters Parameters) (interface{}, error) {
	for _, value := range right.([]interface{}) {
		if strings.HasPrefix(left.(string), value.(string)) {
			return true, nil
		}
	}
	return false, nil
